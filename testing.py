...
net = Network()
net.load_state_dict(torch.load('final_model').state_dict)
...
path2=input()
img1=torch.tensor(format2(path1),device=device,dtype=torch.double).reshape(1,1,100,100)
img2=torch.tensor(format2(path2),device=device,dtype=torch.double).reshape(1,1,100,100)
out1,out2=net(img1/255,img2/255)
confidence = min(out1[0][0],out2[0][0])/max(out1[0][0]/out2[0][0])*100
print('confidence:')
print(confidence)
if confidence > 90:
